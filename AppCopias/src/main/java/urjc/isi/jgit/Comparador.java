...
public class Comparador {
	...
	private static String resultadosCopia(String gitDiffOutput, String url1, String url2, String f1, String f2) {
	   try {
		   ...
		   if  (parts.length == 0) {
			   ...
			   int maxLines = Math.max(f2_nlines, f1_nlines);
			       
			   porcentCopia = (count*100/(float) maxLines);
		   }
...
public String hacerGitDiff(String url1, String url2){
    ...
    try {
        ...
        String resultado = resultadosCopia(output, url1, url2, f1.toString(), f2.toString());	        
	...
	public String compararUltimoCommit (String url1, String url2){
		try {
			...
			if ((latestCommitTime1-latestCommitTime2 >=0 && latestCommitTime1-latestCommitTime2 <=1800)||
					...
					resultado = "\nPosible copia por último commit entre " +nombre1 +" y " 
					+ nombre2 +", no se diferencian en media hora \n";
			}else {
					resultado = "\nNo hay indicios de copia entre " + nombre1 + " y "+ nombre2+
